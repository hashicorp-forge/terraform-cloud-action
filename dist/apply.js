...
function exponentialDelay(retryNumber = 0, error = void 0, delayFactor = 100) {
  const calculatedDelay = __pow(2, retryNumber) * delayFactor;
...
function getRequestOptions(config, defaultOptions) {
  return __spreadValues(__spreadValues(__spreadValues({}, DEFAULT_OPTIONS), defaultOptions), config[namespace]);
}
...
var axiosRetry = (axiosInstance, defaultOptions) => {
  ...
  const responseInterceptorId = axiosInstance.interceptors.response.use(null, (error) => __async(void 0, null, function* () {
    var _a;
    const { config } = error;
    if (!config) {
      return Promise.reject(error);
...
function configureRunCreateOptions(wsID) {
  return {
    ...
    isDestroy: core2.getBooleanInput("is-destroy"),
    message: core2.getInput("message"),
    replaceAddrs: core2.getMultilineInput("replace-addrs"),
    targetAddrs: core2.getMultilineInput("target-addrs"),
    workspaceID: wsID
  };
}
